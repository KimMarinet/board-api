...
public class FileController {
    ...
    @Operation(summary = "파일 목록 조회", description = "/list/(gid)로 목록 조회, /list/(gid)/위치문자열 - <그룹아이디, 위치문자열>로 목록 조회, 파일은 그룹작업이 완료된 파일만 노출")
    @Parameters({
    ...
    @GetMapping("/info/{seq}")
    public FileInfo info(@PathVariable Long seq) {
        FileInfo item = infoService.get(seq);

        return item;
    }

    @Operation(summary = "파일 한개 삭제", description = "파일 등록번호로 삭제")
    @Parameter(name = "seq", required = true, in = ParameterIn.PATH)
    @ApiResponse(responseCode = "200", description = "삭제된 파일 정보가 반환")
    ...
    @Operation(summary = "파일 목록 삭제", description = "그룹아이디 또는 그룹아이디 + 그룹내 위치 구분문장려 조합으로 목록 삭제")
    @Parameters({
            @Parameter(name = "gid", required = true, in = ParameterIn.PATH, description = "그룹아이디"),
            @Parameter(name = "location", in = ParameterIn.PATH, description = "그룹 내에서 위치 구분 문자열")
    })
